import { CalendarEvent, CalendarConfig, SportEvent } from '../types';
...
export class CalendarService {
  ...
  static convertSportEventToCalendarEvent(sportEvent: SportEvent, sportName: string, memberName: string): CalendarEvent {
    const startDateTime = new Date(sportEvent.date);
    const [startHour, startMinute] = sportEvent.startTime.split(':').map(Number);
    startDateTime.setHours(startHour, startMinute, 0, 0);

    const endDateTime = new Date(sportEvent.date);
    const [endHour, endMinute] = sportEvent.endTime.split(':').map(Number);
    endDateTime.setHours(endHour, endMinute, 0, 0);

    return {
      id: `sport-${sportEvent.id}`,
      title: `${sportEvent.title} - ${sportName} (${memberName})`,
      start: startDateTime,
      end: endDateTime,
      description: `${sportEvent.type === 'game' ? 'Game' : 'Practice'} for ${sportName}. Equipment: ${sportEvent.equipment.join(', ')}${sportEvent.notes ? `\n\nNotes: ${sportEvent.notes}` : ''}`,
      location: sportEvent.location,
      color: sportEvent.type === 'game' ? '#d32f2f' : '#2e7d32'
    };
  }

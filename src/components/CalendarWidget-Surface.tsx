...
import { CalendarEvent, CalendarConfig, SportsConfig } from '../types';
...
const CalendarWidget: React.FC<CalendarWidgetProps> = ({ config, onConfigChange, sportsConfig }) => {
  ...
  const loadEvents = async () => {
    ...
    try {
      ...
      // Add sports events if sports config is available
      if (sportsConfig) {
        sportsConfig.sports.forEach(sport => {
          const member = sportsConfig.members.find(m => m.id === sport.familyMemberId);
          const memberName = member?.name || 'Unknown';
          
          // Add practice events
          sport.practiceSchedule.forEach(sportEvent => {
            const calendarEvent = CalendarService.convertSportEventToCalendarEvent(
              sportEvent, 
              sport.name, 
              memberName
            );
          ...
          // Add game events
          sport.gameSchedule.forEach(sportEvent => {
            const calendarEvent = CalendarService.convertSportEventToCalendarEvent(
              sportEvent, 
              sport.name, 
              memberName
